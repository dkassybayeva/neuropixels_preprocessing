...
metadata = dict(
    ...
    rat_name = 'R12',
    date = '20231121',
    behavior_mat_file = '12_AuditoryTuning_1121_2023_Session134911.mat',
    trodes_datetime = '20231121_153453',
    n_probes = 2,
    DIO_port_num = 1,
    task = 'reward-bias',
    sps = sps,
    task_type = 'AuditoryTuning',
    probe_num = 1,
    kilosort_ver = 2.5
)
#----------------------------------------------------------------------#
...
#----------------------------------------------------------------------#
session_paths = dict()
session_paths['rec_dir'] = rec_dir = f'Y:NeuroData/{metadata["rat_name"]}/{metadata["trodes_datetime"]}.rec/'
...
if SPIKES_AND_TTL:
    for probe_i in range(1, metadata['n_probes']+1):
        ...
        tu.create_spike_mat(probe_dir, spike_dir, session_paths['timestamps_dat'], metadata, fs,
